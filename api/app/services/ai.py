...
공간
- 이름: {room.get('name')}
- 목표 온도: {room.get('target_temperature')}°C

판단
- 시각: {decision.get('timestamp')}
- 제어 모드: {decision.get('control_mode')}
- 제안한 동작: {decision.get('content') or decision.get('proposed_action')}
- 실제 전송 여부: {'전송함' if decision.get('executed') else '전송 안 함'}
- 재실 상태: {decision.get('occupancy_state')}
- 그때의 온도: {decision.get('temperature')}
- 그때의 CO2: {decision.get('co2')}
- reason codes: {decision.get('reason_codes')}
...
공간: {stats.get('room_name')} (목표 온도 {stats.get('target_temperature')}°C)
