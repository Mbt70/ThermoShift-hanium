# ThermoShift Agentic Operations Copilot

## 결론

에이전틱 AI는 30초 제어 루프를 대신하는 제어기가 아니라, 운영자가 데이터를
조회하고 실험을 준비하며 이상 원인을 좁히는 **상위 운영 계층**으로 둔다.
실시간 안전 제어는 지금처럼 결정론적 gateway가 담당한다.

```text
센서 → 검증/저장 → RC·PMV·MPC → 안전 정책 → 액추에이터
           ↑              ↑
           └── Copilot ───┘  (조회·설명·시뮬레이션)
                         ↓
                  사람의 명시적 승인
                         ↓
                제한된 변경 API + 감사 로그
```

## 도구 범위

초기 버전은 다음 도구를 제공한다. 구현은 `api/services/copilot_tools.py`,
API 진입점은 `POST /ai/copilot/chat`과
`POST /ai/copilot/tools/{tool_name}`이다.

1. `get_live_snapshot(room_id)` — 최신 센서값, 수신 시각, 품질 플래그
2. `get_recent_decisions(room_id)` — 최근 판단과 저장된 근거
3. `get_experiment_status(room_id)` — 최근 실험 run과 메타데이터
4. `get_model_status()` — 가정값/교정값 여부와 물리 파라미터
5. `simulate_mpc(scenario)` — 장치를 건드리지 않는 what-if 비교
6. `propose_control_action(room_id, action)` — 실행하지 않는 승인 카드
7. `get_data_quality(run_id)` — 표본 연속성·입력 검증·학습 후보 판정
8. `get_experiment_readiness()` — 센서 최신성·중복 run·명령 큐 사전점검
9. `propose_experiment_start(plan_key)` — 시작하지 않는 계획/수동 작업 카드
10. `propose_experiment_stop()` — 중단하지 않는 안전 확인 카드

Gemini가 사용 가능하면 자연어를 구조화된 도구
인자로 계획하고, 사용할 수 없으면 결정론적 한국어 라우터로 폴백한다.
인자 없는 MPC 요청은 최신 센서 조회와 시뮬레이션을 연쇄 실행한다.

이 정도만으로도 “왜 꺼졌나?”, “내일 실험해도 되나?”, “이 run을 학습에
써도 되나?”, “쾌적도 가중치를 바꾸면 어떤 차이가 나나?”에 답할 수 있다.

## 변경 작업 계약

챗봇이 장치 변경을 제안할 수는 있지만 직접 MQTT를 발행하거나 DB를 임의
수정하지 않는다. 변경은 반드시 아래 순서를 따른다.

1. 챗봇이 구조화된 `ActionProposal`을 만든다(현재 구현).
2. API가 허용 목록, 값 범위, 현재 안전 상태를 결정론적으로 검사한다.
3. 화면에 대상·변경값·지속시간·자동 복구값을 보여준다.
4. 로그인한 운영자가 화면 확인 체크와 승인 버튼으로 기존 제어 API에 요청한다.
5. gateway가 한 번에 하나의 명령만 MQTT로 실행한다.
6. ESP32가 `cooling/state`로 실제 릴레이 상태를 회신하면 `acked`로 종결한다.
   120초 안에 상태가 없으면 `timeout`이며, 단순 `sent`를 성공으로 표시하지 않는다.

대시보드는 명령 상태 부분만 2초마다 갱신한다. 따라서 전체 화면을 하얗게
다시 그리지 않고 `요청 접수 → 장치 확인 중 → 동작 확인 완료`를 보여준다.
동일 제안의 네트워크 재시도는 `proposal_id`로 중복 명령 생성을 막는다.

현재 실제 실행 승인은 기존 HVAC 명령에만 연결한다. 실험 시작·중단은 수동
히터의 물리 상태와 OFF 확인을 API가 검증할 수 없으므로 제안 카드까지만
제공한다. 자동 모델 승격, 안전 임계값 변경, 무인 히터 ON, 장시간 액추에이터
ON은 허용하지 않는다. 데모 세션은 항상 읽기 전용이다.

대시보드의 `AI 코파일럿` 화면은 조회 결과의 원본 근거와 사용한 도구를 함께
표시한다. 제어 요청은 즉시 실행되지 않고 승인 카드가 되며, 승인 후에도 API의
소유권 검사와 gateway의 모드·lockout·rate-limit 검사가 최종 권한을 가진다.

## 모델과 프롬프트의 경계

- LLM은 센서 수치나 절감률을 생성하지 않고 도구 결과만 인용한다.
- PMV, 목적함수, RC 예측과 MPC 해는 Python 코드가 계산한다.
- 데이터가 없으면 `unknown`으로 답하고 추정값을 실측처럼 표현하지 않는다.
- LLM 장애·시간초과가 제어 루프와 대시보드 갱신을 막지 않는다.
- 모든 답변에 데이터 시각, 모델 상태, `[MEASURED]/[SIM]` 범위를 붙인다.

## 한이음 시연 장면

운영자가 “현재 냉방을 켜야 하나?”라고 질문하면 Copilot이 최신 센서,
재실확률, PMV proxy, 모델 교정 상태와 MPC what-if를 조회한다. 이후
“현재 데이터에서는 PMV 허용대역이며, 냉방 시뮬레이션의 추가 전력만 증가해
OFF를 권장한다”처럼 근거를 보여준다. 장치 제어 버튼은 별도의 승인 단계로
남는다. 이 장면은 생성형 AI가 제어를 대신한다는 과장 없이도 센서·물리모델·
최적화·인간 승인의 통합을 보여준다.
